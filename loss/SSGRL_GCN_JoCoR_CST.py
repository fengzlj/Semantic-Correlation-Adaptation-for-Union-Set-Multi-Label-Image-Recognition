...
class KLLoss(nn.Module):
    ...
    def forward(self, input1, input2, target, pseudotarget, BCEW, epoch, generateLabelEpoch, forget_rate):
        loss_pick_1 = self.bce_loss(input1, target) 
        loss_pick_2 = BCEW * self.bce_loss(input1, pseudotarget) if epoch >= generateLabelEpoch else \
                 0 * self.bce_loss(input1, pseudotarget)
        loss_pick = ((loss_pick_1 + loss_pick_2) * (1-self.co_lambda) + self.co_lambda * self.kl_loss(input1, input2) + self.co_lambda * self.kl_loss(input2, input1)).cpu() 
