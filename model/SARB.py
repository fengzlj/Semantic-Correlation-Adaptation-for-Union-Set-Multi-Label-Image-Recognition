...
class SARB(nn.Module):
    ...
    def forward(self, input, target=None):
        ...
        # # (batchsize, 10, classnum) -> (batchsize, classnum) (random choose one prototype from ten)

        # mask = torch.rand(target.size()).cuda()
        # mask = mask * (target == 0)
        # mask[torch.arange(target.size(0)), torch.argmax(mask, dim=1)] = 1
        # mask[mask != 1] = 0
        # # (batchsize, classnum) everybatch have one class = 1 (random choose one class which target == 0 from 80)
        ...
        return result, semanticFeature, mixedResult_1, mixedTarget_1
    ...
    def computePrototype(self, train_loader):
        ...
        # features = (10, outputdim, classnum)
        ...
        # prototypes = (batchsize, 10, outputdim)
        # self.prototype = (batchsize, 10, outputdim)
